...
if hasattr(sys, 'frozen') and hasattr(sys, '_MEIPASS'): # running from frozen exe
    appid_path = os.path.join(sys._MEIPASS, 'JLParser_AppID.csv')
else:
    appid_path = os.path.join(os.getcwd(), 'JLParser_AppID.csv')
...
class JL:
    ...
    def __init__(self , args , appid_path, logo = True):
        if not args.quiet:
            self.quiet = False
        ...
        self.res = self.handle_output(output)
...
# callable code:
# iargs class to mimic argparser args for JL class
class iagrs:
    def __init__(self, ifile=None, folder=None):
        self.input_file = ifile
        self.input_dir = folder
        self.output_file = None
        self.quiet = True
        self.output_format = None
        self.appids_file = None
        self.delimiter = None
        self.pretty = None
        if not ifile and not folder:
            raise Exception("JLParser: no input file or folder, or no output file.")

# function to be called from interface
def jlmain(ifile):
    if os.path.isdir(ifile): # call with folder
        args = iagrs(folder=ifile)
        jumplist = JL(args , appid_path, logo=False)
        return json.loads(jumplist.res)
    else:                    # call with single file
        args = iagrs(ifile=ifile)
        jumplist = JL(args , appid_path, logo=False)
        return json.loads(jumplist.res)
# end callable code