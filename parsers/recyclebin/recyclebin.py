...
def decoding(content):
    try:
        test_file_path = content[16:24].hex()
...
def main(file_path):
    ...
    # ==== file size
    result['file_size'] = str( unpack("<Q", unhexlify(content[8:16].hex()))[0] )

    # ==== deleted time
    # date_del = decoding(file_path)
    epoch = datetime.datetime(1601, 1, 1, 0, 0, 0)
    nt_timestamp = unpack("<Q", unhexlify(content[16:24].hex()))[0]
    ...
    if binascii.hexlify(content[0:8] ).startswith( b'01' ):
        # ==== win version
    ...
    result['Path'] = deleted_file_path.replace(b"\x00",b"").decode("utf-8")
    result['File_Name'] = deleted_file_path.replace(b"\x00",b"").split(b'\\')[-1].decode("utf-8")
